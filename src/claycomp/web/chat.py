...
If OPENAI_API_KEY is missing, tell them to add it to their .env file."""
...
async def stream_chat(
  ...
  if not api_key:
    yield _sse({"type": "token", "content": "Add `OPENAI_API_KEY` to your `.env` file to use chat mode."})
  ...
  # Detect enrichment intent and run inline if possible
